...
fdescribe('MoviesComponent', () => {
  ...
  function createMockDataService() {
    const dataServiceMock = {
      getMovies: jasmine.createSpy().and.returnValue(of([
        {
  ...
  beforeEach(async () => {
    dataServiceMock = createMockDataService();
  ...
  it('should return movies info from service', () => {
    ...
    expect(dataServiceMock.getMovies).toHaveBeenCalled();
    expect(component.movies).toEqual(expectedMovies)
  })

  it('should paginate correctly using next and previous buttons', () => {
    component.movies = [
      {
          "title": "American History X",
          "description": "A former neo-nazi skinhead tries to prevent his younger brother from going down the same wrong path that he did.",
    ...
    const nextButtonContainer = fixture.debugElement.query(By.css('.pagination-next'))
    const nextButton = fixture.debugElement.query(By.css('.pagination-next a'))
    ...
    expect(nextButtonContainer.nativeElement).toHaveClass('disabled')

    const previousButton = fixture.debugElement.query(By.css('.pagination-previous a'))
    previousButton.triggerEventHandler('click', null)
    fixture.detectChanges();
    expect(component.paginaActual).toBe(2)

    component.searchMovie('America')
    fixture.detectChanges();
    expect(component.paginaActual).toBe(1)
  })

  it('should test movies filters', () => {
    component.movies = [
      {
          "title": "American History X",
          "description": "A former neo-nazi skinhead tries to prevent his younger brother from going down the same wrong path that he did.",
    ...
    const initialMovies = fixture.debugElement.queryAll(By.css('article.movies section'))
    expect(initialMovies).toHaveSize(10)

    component.searchMovie('America');
    fixture.detectChanges();
    const moviesFound = fixture.debugElement.queryAll(By.css('article.movies section'))
    expect(moviesFound).toHaveSize(2)

    component.dateMovie(1998)
    fixture.detectChanges();
    const moviesFoundWithDate = fixture.debugElement.queryAll(By.css('article.movies section'))
    expect(moviesFoundWithDate).toHaveSize(1)

    component.filterText = ''
    fixture.detectChanges();
    const moviesFoundInReset = fixture.debugElement.queryAll(By.css('article.movies section'))
    expect(moviesFoundInReset).toHaveSize(2)
